...
### 3단계: 경로 탐색 엔진 (Backend/Frontend) (Phase 3)
1.  **단순 경로 탐색 (Categorical Navigation)**:
    -   복잡한 OpenCV/A* 대신 `map_zones` 테이블 활용.
    -   `findZoneByCategory(zone_list, category_name)` 로직 구현.
    -   `getZoneCenter(zone)`로 목적지 좌표 계산.
    -   직선(Linear) 또는 꺾은선(Kiosk -> Center -> Zone) 연결.
2.  **API 엔드포인트**:
    -   기존 `/api/map/zones` 활용 (전체 존 데이터 로드 후 프론트에서 처리).
...
4.  [x] **API**: `backend/api.py`에 경로 탐색 엔드포인트 추가 (Map Zones API로 대체).
5.  [ ] **UI (MapNavigation)**: `frontend/src/components/MapNavigation.tsx` 생성 및 `MapEditor` 데이터 연동.
6.  [ ] **통합**: 검색 API 결과(`category`)와 맵 에디터 구역(`zone.name`) 매칭 로직 구현.
